...
<< load.data, echo=FALSE, message=FALSE, paged.print=FALSE, results=tex>>=
library(BOBaFIT)
data("segments")
...
<< ComputeNormalChromosome, echo=TRUE, results=hide, fig.show='hide'>>=
library(BOBaFIT)
chr_list <- computeNormalChromosomes(segments = segments, perc = 0.15)
@
<< XomputeNormalChromosome,results=tex >>=
chr_list
@
