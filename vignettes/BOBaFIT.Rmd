...
The R package BoBafit is composed of three functions which allow the refit
and the recalibration of copy number profile of tumor sample. 
...
The dataset needs an extra step for being the perfect BOBaFIT input. All functions work with CN value of each segment, so it must to be calculated from the segment mean or LogR values. We suggest with the following forumula:
```{r}
#When data coming from SNParray platform are used, the user have to apply the compression factor in the formula (0.55). In case of WGS/WES data, the correction factor is equal to 1.

TCGA_BRCA_CN_segments$CN <- 2^(TCGA_BRCA_CN_segments$Segment_Mean/0.55 +1)
options(scipen = 999) 
```
...
The chromosome list is a vector that contains the chromosomal arms considered "normal" in the cohort of samples tested. Chromosomes included in the list should be selected when their CN values are subject to minimal fluctuation and tend to remain within the diploid range in the analyzed tumor. *ComputeNormalChromosome* allows to set the chromosomal alteration rate (`tollerance_val`). For a more stringent analysis, we suggest an alteration rate of 5% (0.5) ; on the contrary, for a more permissive analysis, we suggest as maximum rate 20-25% (0.20-0.25) . The function input is a sample cohort with their segments.

Here we performed the function in the data set `TCGA_BRCA_CN_segments`, using an alteration rate of 15%.

```{r computeNormalChromosome, fig.show='hide', include=FALSE, results='hide'}
library(BOBaFIT)
chr_list <- computeNormalChromosomes(segments = TCGA_BRCA_CN_segments, tollerance_val = 0.15)
```
